...
class NotificationSettingsBloc
  ...
  Future<void> _onGetDeviceToken(
    ...
    tokenResult.fold(
      (failure) => emit(NotificationSettingsFailure("خطأ اثناء الحصول على توكن الجهاز")),
  ...
  Future<void> _onSubscribeToTopic(
    ...
    result.fold(
      (failure) => emit(NotificationSettingsFailure("خطأ اثناء الاشتراك في التوبيك")),
      (_) {
  ...
  Future<void> _onUnsubscribeFromTopic(
    ...
    result.fold(
      (failure) => emit(NotificationSettingsFailure("خطأ اثناء الالغاء الاشتراك في التوبيك")),
      (_) {
        if (state is NotificationSettingsLoaded) {
          final currentState = state as NotificationSettingsLoaded;
          final updatedTopics = List<String>.from(currentState.subscribedTopics)
