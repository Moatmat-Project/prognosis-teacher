...
import 'package:moatmat_teacher/Presentation/outer_tests_results/views/answers_percentage_v.dart';
...
import '../state/cubit/outer_test_results_cubit.dart';

class OuterTestResultsView extends StatefulWidget {
  const OuterTestResultsView({super.key, required this.test});
  final OuterTest test;
  @override
  State<OuterTestResultsView> createState() => _OuterTestResultsViewState();
}

class _OuterTestResultsViewState extends State<OuterTestResultsView> {
  @override
  void initState() {
    context.read<OuterTestResultsCubit>().init(widget.test);
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: BlocBuilder<OuterTestResultsCubit, OuterTestResultsState>(
        builder: (context, state) {
          if (state is OuterTestResultsInitial) {
            return Scaffold(
              appBar: AppBar(
                title: Text(
              ...
              body: RefreshIndicator(
                onRefresh: () async {
                  context.read<OuterTestResultsCubit>().update();
                  return;
                },
                child: Column(
                  children: [
