...
class StudentsDSimpl implements StudentsDS {
  ...
  Future<List<UserData>> getMyStudents({required bool update}) async {
    ...
    final userIds = myResults.map((e) => e["user_id"] as String).toSet().toList();
    ...
    //
    List<UserData> users = allUsers.map((e) {
      return UserDataModel.fromJson(e);
    }).toList();
    //
    users = removeDuplicateUuids(users);
    //
    return users;
  }
  ...
  Future<Unit> addResults({required List<Result> results}) async {
    ...
    List list = results.map((e) {
      return ResultModel.fromClass(e).toJson();
    }).toList();
    //
